      ...
      <div className="marquee-track marquee-track--videos items-start">
        {doubled.map((video, i) => (
          <a
            ...
            className="group relative block w-80 shrink-0 overflow-hidden rounded-xl bg-surface shadow-lg transition-transform hover:scale-[1.03] sm:w-96"
          >
            <div className="relative aspect-video w-full overflow-hidden">
              ...
              <div className="absolute inset-0 flex items-center justify-center bg-black/20 transition-colors group-hover:bg-black/40">
                <span className="flex h-16 w-16 items-center justify-center rounded-full bg-white/90 shadow-lg transition-transform group-hover:scale-110">
                  <svg viewBox="0 0 24 24" className="ml-1 h-7 w-7 fill-darkest">
                    <path d="M8 5v14l11-7z" />
                  </svg>
                </span>
              </div>
            </div>
            <p className="truncate px-4 py-3 text-[16px] font-medium text-white/90">{video.title}</p>
          </a>
        ))}
      </div>
    </div>
